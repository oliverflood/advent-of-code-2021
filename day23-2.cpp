...
map<vector<vector<int>>, int> reference;
...
bool recurse(vector<vector<int>> board, int score) {
    //cout << "hi" << endl;
    vector<pair<vector<vector<int>>, int>> boardList = listLegals2(board, score);
    if (boardList.size() == 0 && !isComplete(board)) {
        return(false);
    }
    ...
    for (int i = 0; i < boardList.size(); ++i)
    {
        ...
        if (!reference.count(boardList[i].first)) {
            reference[boardList[i].first] = boardList[i].second;
        ...
        // else {
        //     reference[boardList[i].first] = max(boardList[i].second, reference[boardList[i].first]);
        //     //reference[boardList[i].first] = 3;
        // }
    }
    boardList.clear();
    return(false);
