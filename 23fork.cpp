...
bool blocked(vector<vector<int>> board, int x1, int y1, int x2, int y2) {
    ...
    for (int i = 1; i <= y2; ++i)
    {
        if (board[i][x2] != 0 && board[i][x2] != board[x1][y1]) {
            ans = true;
        }
    }
...
int main()
{
    ...
    // print(precipitate(board, 0).first);
    // cout << precipitate(board, 0).second << endl;
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        {
            vector<pair<vector<vector<int>>, int>> currentList = listLegals(boardList[k].first, boardList[k].second);
    ...
    cout << (float) (pow(3, 2) * (9 % 4)) / 2 + (int)(2.5) << endl;
