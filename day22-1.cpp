...
int main()
{
    ...
    vector<pair<vector<vector<int>>, bool>> steps = getInput(60); //5035531490130659
    ...
    for (int i = 0; i < steps.size(); ++i)
    {
        ...
        {
            if (true) { //!(k==0 && i==0)
                if (steps[i].second == steps[k].second) {
    ...
    for (int i = 0; i < steps.size(); ++i)
    {
        testArea += ((long long)(steps[i].second==0?0:1))*area(steps[i].first);
    }
    cout << endl << "maxArea: " << testArea << endl;
    for (int i = 0; i < list2.size(); ++i)
    {
        testArea += ((long long)(list2[i].second==0?-1:1))*area(list2[i].first);
    }
